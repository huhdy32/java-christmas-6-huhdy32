# 소개
사용자에게 식당 예약 서비스를 제공하는 어플리케이션입니다. 
예약 날짜와 주문 메뉴를 차례로 입력받아 예약 서비스를 제공하며 유효하지 않은 입력에 대한 에러처리와, 재입력을 수행합니다.
유효한 입력에 대해 어플리케이션은 해당 주문에 적용가능한 이벤트를 물색하고 사용자에게 예약된 정보, 적용되는 이벤트에 대한 정보를 출력하도록 합니다.
...
- 출력 모델
출력될 데이터드렝 대한 값을 도메인, 혹은 서비스 에서 직접 호출할것인가 아니면 출력 모델을 따로 구현하여 해당 모델의 메소드를 호출할 것인가?
전자의 경우 뷰와 비즈니스 모델의 결합도가 올라감으로 출력용 모델을 구현하여 이를 뷰에서 호출하도록 하자. 

- 파싱 방법 
배열을 최대한 자제하라고 헀지만, 파싱과정에서 생기는 문자열들을 스트림, 혹은 리스트로 구현하는데 어려움을 겪고 있다.
일단 배열로 구현하고, 이를 리팩토링 하도록 하자. 
